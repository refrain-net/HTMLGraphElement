...
class HTMLGraphElement extends HTMLCanvasElement {
  ...
  get names () { return this.#elements.map(({name}) => name); }
  ...
  render (...indices) {
    ...
    indices = indices.filter(index => 0 <= index && index < elements.length);
    if (indices.length === 0) indices = [...elements.keys()];

    const mapping = (value, index) => index % 2 === 0 ? (value * (originX === 0 ? 1 : 2) - minX) / rangeX + originX : (value * (originY === 0 ? 1 : 2) - minY) / rangeY + originY;
    const filtering = (value, index, array) => {
      value = Math.abs(array[index - index % 2]);
      return 0 <= value && value <= 1;
    };

    elements.filter((e, index) => indices.length === 0 || indices.includes(index)).map(({color, data}) => ({
      color,
